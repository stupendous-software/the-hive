...
clone supports modern AI protocols for interoperability.

## MCP (Model Context Protocol)

### As Server

clone can expose its tools as an MCP server. Enable in Settings → MCP Server. The server runs via streamable HTTP (default port configurable). External clients can then call clone’s tools.

### As Client

clone can connect to external MCP servers to extend its capabilities. Configure the server URL in Settings → MCP Clients. Once added, the server’s tools become available to the agent.
...
FastA2A protocol allows clone instances to communicate directly.
